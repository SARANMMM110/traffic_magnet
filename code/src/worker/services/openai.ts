...
function generateLandingPagePrompt(blueprint: any): string {
  // Extract input fields for dynamic form generation
  const inputs = Array.isArray(blueprint.inputs_required) && blueprint.inputs_required.length > 0
    ? blueprint.inputs_required
    : ['Primary Input', 'Secondary Input', 'Category Selection'];
  ...
  return `You are a principal SaaS product designer and frontend engineer specializing in premium AI startup landing pages.

Create a COMPLETE, PRODUCTION-READY HTML document for a premium AI tool landing page.

CRITICAL OUTPUT REQUIREMENTS:
- Output ONLY raw HTML. NO markdown code fences (no \`\`\`html or \`\`\`). NO explanations. NO preamble.
- Start directly with <!DOCTYPE html>
- Single complete HTML file with embedded CSS and JavaScript

QUALITY STANDARD:
Create a landing page that feels like a funded AI startup product (Linear, Stripe, Framer, Notion quality).
The page must feel premium, modern, immersive, conversion-focused, and professionally designed.
NEVER feel generic, AI-generated, template-based, or Bootstrap-like.

TECHNICAL REQUIREMENTS:
- Responsive design (360px mobile to 1920px desktop)
- Google Fonts: Inter with weights 400, 500, 600, 700, 800
- Single <style> block with clean, reusable CSS classes
- Single <script> block for form interactivity
- No external dependencies, frameworks, or CDN links except Google Fonts
- Total file size under 75KB for stable preview rendering
...
Typography:
- Headings: Inter, 700-800 weight, tight letter-spacing
...
Components:
- Border radius: 16px-32px (modern, generous curves)
- Shadows: soft, layered (0 20px 40px -15px rgba(0,0,0,0.1))
...
2. Hero Section
   - Badge component with AI/premium positioning
   - Large headline (60-80px desktop, emphasize value)
   - Supporting description (18-20px, 600px max-width)
   - Dual CTA buttons (primary gradient + outline)
   - Trust indicators (3-4 small proof points with icons)
   - Background: subtle gradient radial effects

3. Tool Card Section (MOST IMPORTANT - MUST PASS VALIDATION)
   This is the interactive core. Must contain these EXACT class names:

   <section class="tool-section">
     <div class="tool-card">
       <div class="tool-header">
         <span class="tool-ai-badge">✨ AI-Powered Analysis</span>
         <h2>Tool Title: ${blueprint.title}</h2>
       ...
       <form id="tool-form" class="tool-form-grid">
         ${inputs.map((input: any, idx: number) => {
           const fieldName = typeof input === 'string' ? input : (input.name || input.label || `Field ${idx + 1}`);
         ...
         <div class="tool-field-card">
           <label>
             <span class="tool-icon-chip">🎯</span>
             ${fieldName}
           </label>
           <div class="tool-select-wrap">
             <select name="field${idx}" required>
         ...
         <div class="tool-field-card">
           ...
           <input type="text" name="field${idx}" placeholder="Enter ${fieldName.toLowerCase()}..." required>
         </div>`;
           }
         }).join('\n')}
         ...
         <button type="submit" class="tool-primary-button">
           ...
           <svg width="20" height="20" viewBox="0 0 20 20" fill="none"><path d="M4 10h12m0 0l-4-4m4 4l-4 4" stroke="currentColor" stroke-width="2" stroke-linecap="round"/></svg>
         </button>
       </form>
       
       <div class="tool-result-dashboard" id="results" style="display: none;">
         <h3>Your AI Analysis Results</h3>
         
         <div class="tool-score-ring">
           <svg viewBox="0 0 100 100">
             <circle cx="50" cy="50" r="40" fill="none" stroke="#E5E7EB" stroke-width="8"/>
             <circle cx="50" cy="50" r="40" fill="none" stroke="#6E57E0" stroke-width="8" stroke-dasharray="251.2" stroke-dashoffset="62.8" transform="rotate(-90 50 50)"/>
           </svg>
         ...
         <div class="results-grid">
           <div class="tool-insight-card">
             <h4>🎯 Key Insight</h4>
             <p>Your inputs show strong potential for ${blueprint.purpose || 'growth and optimization'}.</p>
           </div>
           
           <div class="tool-recommendation-card">
             <h4>💡 Recommendation</h4>
             <p>Focus on ${blueprint.target_keywords?.[0] || 'strategic optimization'} to maximize results.</p>
           </div>
           
           <div class="tool-monetization-card">
             <h4>💰 Monetization Path</h4>
             <p>${blueprint.monetization_strategy || 'Multiple revenue opportunities identified based on your profile.'}</p>
           </div>
         </div>
       </div>
     </div>
...
4. Benefits Section
   ...
   - Use blueprint.features or default benefits
   - Modern card design with hover effects

5. How It Works / Process
   - 3-4 step cards in horizontal layout
   - Numbered indicators
   - Clear, concise steps
   - Visual connection between steps

6. Trust / Social Proof
   - Testimonial cards or outcome metrics
   - 2-3 items
   - Authentic, not fabricated numbers
   - Focus on outcomes, not vanity metrics

7. FAQ Section
   - <details>/<summary> accordion components
   - 5-6 common questions
   - Modern styling with smooth expand/collapse
   - Related to blueprint.purpose and use case
...
document.getElementById('tool-form').addEventListener('submit', function(e) {
  ...
  // Show loading state
  const button = this.querySelector('.tool-primary-button');
...
CSS REQUIREMENTS:
- All tool-* classes must be defined with premium styling
- .tool-card: white background, large border-radius, soft shadow, generous padding
- .tool-field-card: subtle background, border, rounded corners, clean layout
- .tool-select-wrap: custom select styling (hide default arrow, add custom arrow)
- .tool-primary-button: gradient background, large padding, hover lift effect
- .tool-result-dashboard: grid layout for cards, smooth reveal animation
- .tool-score-ring: circular SVG progress indicator
- All cards have hover states with subtle lift

VALIDATION CHECKLIST (THESE MUST EXIST):
✓ tool-card
✓ tool-header
✓ tool-ai-badge
✓ tool-progress
✓ tool-form-grid
✓ tool-field-card (for each input/select)
✓ tool-icon-chip (in each field label)
✓ tool-select-wrap (wrapping each <select>)
✓ tool-primary-button
✓ tool-result-dashboard
✓ tool-insight-card
✓ tool-recommendation-card
✓ tool-monetization-card
✓ tool-score-ring or tool-score-card

BLUEPRINT DATA:
Title: ${blueprint.title}
Description: ${blueprint.description || blueprint.purpose}
Purpose: ${blueprint.purpose}
Category: ${blueprint.category || 'Business Tool'}
Target Keywords: ${Array.isArray(blueprint.target_keywords) ? blueprint.target_keywords.join(', ') : 'optimization, growth, analysis'}
Features: ${JSON.stringify(features)}
Monetization: ${blueprint.monetization_strategy || 'Multiple revenue streams'}
...
function injectPremiumFieldPolish(html: string): string {
  if (!/<\/style>/i.test(html) || html.includes("trafficmagnet-premium-polish")) {
    return html;
  }

  const premiumPolish = `

/* trafficmagnet-premium-polish */
:where(.experience-shell) {
  background:
    radial-gradient(circle at 8% 4%, rgba(124, 92, 252, 0.16), transparent 32%),
    radial-gradient(circle at 92% 12%, rgba(14, 165, 233, 0.13), transparent 28%),
    linear-gradient(180deg, #fbfcff 0%, #f6f7fb 46%, #ffffff 100%);
}
...
:where(.hero-composition, .ai-product-preview, .tool-panel, .configurator-card, .analysis-workspace) {
  position: relative;
  overflow: hidden;
  border: 1px solid rgba(226, 232, 240, 0.92);
  background:
    linear-gradient(135deg, rgba(255, 255, 255, 0.96), rgba(248, 250, 252, 0.86)),
    radial-gradient(circle at 16% 0%, rgba(124, 92, 252, 0.12), transparent 34%);
  box-shadow: 0 28px 80px rgba(15, 23, 42, 0.10), inset 0 1px 0 rgba(255, 255, 255, 0.92);
  backdrop-filter: blur(18px);
}

:where(.ai-product-preview, .configurator-card) {
...
:where(.field-card) {
  ...
  box-shadow: 0 16px 40px rgba(15, 23, 42, 0.06), inset 0 1px 0 rgba(255, 255, 255, 0.95);
  transition: transform 180ms ease, box-shadow 180ms ease, border-color 180ms ease;
}

:where(.field-card:hover) {
  transform: translateY(-2px);
  border-color: rgba(124, 92, 252, 0.32);
  box-shadow: 0 22px 52px rgba(15, 23, 42, 0.09);
}

:where(.field-card label) {
  display: flex;
  align-items: center;
  gap: 8px;
  margin-bottom: 10px;
...
:where(.field-card input, .field-card select, .field-card textarea, .configurator-card input, .configurator-card select, .configurator-card textarea) {
  width: 100%;
  min-height: 54px;
...
:where(.field-card select, .configurator-card select) {
  appearance: none;
  background-image:
    linear-gradient(45deg, transparent 50%, #6d5dfb 50%),
    linear-gradient(135deg, #6d5dfb 50%, transparent 50%),
    linear-gradient(180deg, rgba(255, 255, 255, 0.98), rgba(248, 250, 252, 0.82));
  background-position:
    calc(100% - 22px) 50%,
    calc(100% - 16px) 50%,
...
:where(.field-card input:focus, .field-card select:focus, .field-card textarea:focus, .configurator-card input:focus, .configurator-card select:focus, .configurator-card textarea:focus) {
  border-color: rgba(109, 93, 251, 0.82);
  background: #ffffff;
  box-shadow: 0 0 0 5px rgba(109, 93, 251, 0.13), 0 14px 30px rgba(15, 23, 42, 0.08);
  transform: translateY(-1px);
}

:where(.configurator-card button, .tool-panel button, .primary-action, button[type="submit"]) {
  min-height: 56px;
  border: 0;
  border-radius: 999px;
  padding: 0 24px;
  ...
  letter-spacing: -0.02em;
  background:
    linear-gradient(135deg, #111827 0%, #312e81 45%, #6d5dfb 100%);
  box-shadow: 0 22px 50px rgba(79, 70, 229, 0.30), inset 0 1px 0 rgba(255, 255, 255, 0.22);
  cursor: pointer;
  transition: transform 180ms ease, box-shadow 180ms ease, filter 180ms ease;
}

:where(.configurator-card button:hover, .tool-panel button:hover, .primary-action:hover, button[type="submit"]:hover) {
  transform: translateY(-2px);
  filter: saturate(1.08);
  box-shadow: 0 28px 64px rgba(79, 70, 229, 0.38), inset 0 1px 0 rgba(255, 255, 255, 0.24);
}

:where(.signal-card, .strategy-card, .monetization-card) {
  border-radius: 22px;
  ...
  return html.replace(/<\/style>/i, `${premiumPolish}\n</style>`);
}
...
function normalizeGeneratedHtml(html: string): string {
  let normalized = html
    ...
    .replace(/border-radius:\s*8px/gi, "border-radius: 14px")
    .trim();
...
@media (max-width: 640px) {
  ...
  const aiProductPreviewFallback = `
    ...
    <div class="preview-header">
      <div>
        <p class="eyebrow">AI opportunity intelligence</p>
        <h3>Growth Strategy Console</h3>
      </div>
      <div class="score-meter" aria-label="Opportunity readiness">
        <span></span>
      </div>
    </div>
    <div class="preview-grid">
      <article class="signal-card">
        ...
        <p>Prioritize long-tail questions, comparison pages, and conversion-focused content clusters.</p>
      </article>
      <article class="signal-card">
        <small>Conversion gap</small>
        <strong>Lead capture can be stronger</strong>
        <p>Use a guided assessment, proof-led CTA, and segmented follow-up offer.</p>
      </article>
      <article class="strategy-card">
        <small>AI recommendation</small>
        <strong>Launch with an authority-first funnel</strong>
        <p>Pair the interactive report with trust signals, use-case pages, and a monetization roadmap.</p>
      </article>
      <article class="monetization-card">
        <small>Revenue path</small>
        <strong>Consulting, affiliate, or productized offer</strong>
        <p>Route qualified visitors into a clear next step based on urgency and business maturity.</p>
      </article>
    </div>`;

  normalized = fillEmptyClassContainer(normalized, "ai-product-preview", aiProductPreviewFallback);
  normalized = injectPremiumFieldPolish(normalized);
...
function ensureEmbedSemanticForm(html: string): string {
  ...
  const toolPanelOpen =
    /<div\b[^>]*\bclass\s*=\s*["'][^"']*\btaf-tool-panel\b[^"']*["'][^>]*>/i;
  const insightOpen =
    /<div\b[^>]*\bclass\s*=\s*["'][^"']*\btaf-insight-dashboard\b[^"']*["'][^>]*>/i;
  ...
  if (
    !toolMatch ||
    !insightMatch ||
    toolMatch.index === undefined ||
    insightMatch.index === undefined
  ) {
    return html;
  }
...
function assertPremiumHtmlQuality(html: string, mode: HtmlQualityMode): void {
  ...
  if (mode === "standalone") {
    ...
    if (presentPremiumSignals.length < 6) {
      violations.push(`standalone page needs more premium AI SaaS sections (${presentPremiumSignals.length}/6 found)`);
    }
    ...
    const emptyCriticalContainers = [
      ...
      const emptyContainerPattern = new RegExp(
        `<(div|section|footer)([^>]*class=["'][^"']*\\b${className}\\b[^"']*["'][^>]*)>\\s*</\\1>`,
        "i"
      );
      return emptyContainerPattern.test(html);
    });

    if (emptyCriticalContainers.length > 0) {
      violations.push(`empty placeholder containers: ${emptyCriticalContainers.join(", ")}`);
    }
...
Premium field and interaction requirements:
...
- Inputs must use large rounded controls, soft inner shadow, hover lift, focus glow, and clear active state. They must never look like default browser inputs.
- Select controls must be custom-styled with a premium arrow treatment, not plain operating-system dropdown styling.
- The primary submit button must feel like a premium AI action button: gradient surface, strong typography, hover elevation, and concise value-driven text.
- Add small progress/readiness elements in the configurator such as a step indicator, readiness bar, or "AI analysis path" row.
- The tool area should be one of the most visually impressive parts of the page, with layered cards, glass-light surfaces, and excellent spacing.

Premium visual system requirements:
...
- Use clamp() for headline and section typography, not fixed small headings.
- Use section spacing with clamp(72px, 10vw, 132px) and card spacing with clamp(22px, 4vw, 42px).
- Include subtle decorative orbs, gradient borders, glassmorphism panels, and layered shadows without making the page noisy.
- Add tasteful micro-interactions: hover lift on cards, focus-visible states on fields/buttons, and reduced-motion support.

Standalone reference brief:
- Use this quality target and structure when generating standalone pages: "Create a premium light UI for a digital nomad destination matcher with form inputs, result chips, steps, benefits, FAQ, and CTA — responsive, purple accent, glassmorphism, 12 destinations data, match logic, single HTML."
...
- Organize inputs into progressive steps (Audience, Business Goal, Offer Details, Generate Insights) using field groups or panels; avoid one endless vertical stack of every field at once.
- Include Back/Next buttons, current step state, and a final "Generate Insights" (or similar) action that submits the same form via JavaScript.
...
- Target Framer/Webflow premium template quality: strong spacing rhythm, Plus Jakarta Sans or Inter, glassmorphism, layered cards, elegant gradients, subtle shadows, conversion-focused blocks, realistic dashboard-style results (qualitative insight cards, not empty metric shells).
